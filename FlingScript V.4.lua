--[[
    Fling Script - UN Style UI v2
    Multi-target fling with UNScripts visual polish
    Stripped: no categories, no dropdowns, no Home tab
]]
    ...
    for k, v in pairs(props) do
        ...
        if typeof(v) == "Color3" then
            for cKey, cVal in pairs(C) do
                if v == cVal and cKey ~= "toggle_on" and cKey ~= "dot_red"
                    and cKey ~= "dot_yel" and cKey ~= "dot_grn" and cKey ~= "dot_blue" then
                    tags[k] = cKey
                    break
                end
            end
...
local SettingsBtn = makeHeaderIcon(-64, "⚙")
local ModsBtn     = makeHeaderIcon(-36, "⚡")
...
local SelectionFrame = Make("Frame", {
    Size = ud2(1,-24,0,250), Position = ud2(0,12,0,62),
...
local ButtonsFrame = Make("Frame", {
    Size = ud2(1,-24,0,120), Position = ud2(0,12,0,318),
...
-- ═══════════════════════════════════════════════════════════
--  SETTINGS PAGE (UNscript-style with tab bar animation)
-- ═══════════════════════════════════════════════════════════
...
Make("UIStroke", {Color = rgb(0,0,0), Thickness = 1, Transparency = 0.7, Parent = LightModeKnob})
...
local SliderKnob = Make("TextButton", {
    ...
    BackgroundColor3 = C.knob, BorderSizePixel = 0, Text = "", Parent = SliderTrack,
})
...
local SliderHitbox = Make("TextButton", {
    ...
    BackgroundTransparency = 1, Text = "", ZIndex = SliderKnob.ZIndex - 1, Parent = SliderTrack,
})

-- ═══════════════════════════════════════════════════════════
--  MODIFIERS PAGE (flat, no dropdowns)
-- ═══════════════════════════════════════════════════════════
...
local PowerKnob = Make("TextButton", {
    Size = ud2(0,14,0,14), Position = ud2(powerFrac,-7,0,-4),
    BackgroundColor3 = C.knob, BorderSizePixel = 0, Text = "", Parent = PowerTrack,
})
Make("UICorner", {CornerRadius = ud(1,0), Parent = PowerKnob})
Make("UIStroke", {Color = rgb(80,80,80), Thickness = 1, Transparency = 0.3, Parent = PowerKnob})

local powerDragging = false
local PowerHitbox = Make("TextButton", {
    Size = ud2(1,0,1,20), Position = ud2(0,0,0,-7),
    BackgroundTransparency = 1, Text = "", ZIndex = PowerKnob.ZIndex - 1, Parent = PowerTrack,
})
...
Make("UIStroke", {Color = rgb(0,0,0), Thickness = 1, Transparency = 0.7, Parent = GhostKnob})
...
Make("UIStroke", {Color = rgb(0,0,0), Thickness = 1, Transparency = 0.7, Parent = SafeRetKnob})
    ...
    for _, player in ipairs(PlayerList) do
        if player ~= LocalPlayer then
            ...
            local PlayerEntry = Make("Frame", {
                ...
                Visible = matchesSearch, Parent = PlayerScrollFrame,
            })
            ...
            ClickArea.MouseButton1Click:Connect(function()
                ...
                if currentTime - lastClickTime < 0.25 then
                    for _, p in pairs(Players:GetPlayers()) do
                        if p ~= LocalPlayer then SetPlayerSelection(p, false) end
                    end
            ...
            PlayerCheckboxes[player.Name] = {
                Entry = PlayerEntry, Checkmark = Checkmark,
                StatusDot = StatusDot, SpectateBtn = SpectateBtn,
            }
        end
    end
...
SearchBox:GetPropertyChangedSignal("Text"):Connect(function() RefreshPlayerList() end)

local function ToggleAllPlayers(select)
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then SetPlayerSelection(player, select) end
    end
end
...
local function SkidFling(TargetPlayer)
    ...
    if cfgGhostFling then DestroyGhostPart() end
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if root then
                ...
                if dist < shortestDist then shortestDist = dist; nearestPlayer = p end
            end
        end
    end
...
-- ═══════════════════════════════════════════════════════════
--  WINDOW CONTROLS (UNscript-style drag & pill)
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
--  DRAGGING (UNscript-style with GuiInset)
-- ═══════════════════════════════════════════════════════════
...
targetKeyConn = UserInputService.InputBegan:Connect(function(input, processed)
    if not Gui or not Gui.Parent then targetKeyConn:Disconnect(); return end
    if processed then return end
...
Players.PlayerAdded:Connect(function(player)
    if autoSelectEnabled then task.wait(0.5); SetPlayerSelection(player, true) end
    RefreshPlayerList()
end)
    ...
    if not processed and not isListeningForKeybind and not isListeningForTargetKey
        ...
        if input.KeyCode == comboKey2 and comboKey1 ~= Enum.KeyCode.Unknown and comboKey2 ~= Enum.KeyCode.Unknown then
            if lastComboKey1Press > 0 then
                ...
                if not FlingActive then StartFlingOnce() end
            end
        end
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        if powerDragging then powerDragging = false; triggerAutoSave() end
    end
end)
...
print("[FlingScript] Loaded (UN Style v2)")
