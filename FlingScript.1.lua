...
-- We store press position as a Vector2 so the magnitude check is apples-to-apples.
local pillDragging = false

local pillDragOffset = Vector2.new()

local pillPressPos = nil   -- Vector2: screen position when finger/mouse went down

local pillDragMoved = false

local PILL_DRAG_THRESHOLD = 6  -- pixels of movement before we treat it as a drag
...
-- ╔══════════════════════════════════════════════╗
-- ║  PILL: drag to move, short press to expand   ║
-- ╚══════════════════════════════════════════════╝
...
PillUI.InputBegan:Connect(function(input)
    ...
    or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Store press position as a plain Vector2 so we can do 2-D distance checks.
        ...
        pillPressPos   = Vector2.new(mousePos.X, mousePos.Y)



        -- Offset = pill anchor world pos  minus  current mouse pos.
        -- This keeps the pill from snapping its centre to the cursor.

        local pillAnchorPos = PillUI.AbsolutePosition + PillUI.AbsoluteSize * PillUI.AnchorPoint

        pillDragOffset = pillAnchorPos - mousePos - insetOff
...
PillUI.InputEnded:Connect(function(input)
    ...
    or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- If the finger barely moved it was a TAP → open the window.
        if not pillDragMoved then
...
-- Main window dragging (header area)
...
dragConn = UserInputService.InputEnded:Connect(function(input)
    ...
    or input.UserInputType == Enum.UserInputType.Touch then
        ...
        pillDragging = false   -- safety: release pill drag globally too
...
renderConn = RunService.RenderStepped:Connect(function()
    ...
    -- Move the pill (only after the threshold is crossed)

    if pillDragging then

        local mousePos = UserInputService:GetMouseLocation()

        local mp2      = Vector2.new(mousePos.X, mousePos.Y)



        -- Cross the threshold? Lock in as a drag so release won't open the window.

        if pillPressPos and (mp2 - pillPressPos).Magnitude > PILL_DRAG_THRESHOLD then
        ...
        if pillDragMoved then

            local newPos      = mousePos + pillDragOffset + insetOff

            PillUI.Position   = UDim2.fromOffset(newPos.X, newPos.Y)
...
keybindConn = UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if isListeningForKeybind and input.UserInputType == Enum.UserInputType.Keyboard then
        ...
        elseif #tempKeys >= 2 then
            ...
            activeKeybind[1] = tempKeys[1]

            activeKeybind[2] = tempKeys[2]

            KeybindBtn.Text  = "[ " .. activeKeybind[1].Name .. " + " .. activeKeybind[2].Name .. " ]"
    ...
    local key1held  = UserInputService:IsKeyDown(activeKeybind[1])

    local key2match = input.KeyCode == activeKeybind[2]



    if key1held and key2match then
